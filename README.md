...
Tabulation System for Hunk and Babe at Jill Chester Resort (Balatan, Camarines Sur)
...
3. Clone or download this repository to your XAMPP **htdocs** folder.
   The final path should be `path_to/xampp/htdocs/hb`.
...
5. Inside [phpMyAdmin](http://localhost/phpmyadmin),
   create a MySQL database named `hb` and import [hb.sql](hb.sql) into it.

6. Open the terminal and navigate to the project directory **hb**.
...
9. Open your web browser and access <http://localhost:5195/hb/> to view the application.
...
2. Inside [phpMyAdmin](http://localhost/phpmyadmin),
   create a MySQL database named `test-hb` and import [hb.sql](hb.sql) into it.
...
2. Access the application by visiting `http://[host_name]/hb`,
   where `host_name` is the **IP address** or **host name** of the server in the network.
   For example:
     - <http://localhost/hb>
     - <http://192.168.1.99/hb>


#### Admin Dashboard
Access the **admin dashboard** at <http://localhost/hb/app>.
